...
public sealed class EfSalesInvoiceDirectoryService : ISalesInvoiceDirectoryService
{
    private readonly AppDbContext _dbContext;
    private readonly ICustomerDirectoryService _customerDirectoryService;

    public EfSalesInvoiceDirectoryService(AppDbContext dbContext, ICustomerDirectoryService customerDirectoryService)
    {
    ...
    public SalesInvoiceListViewModel GetList(string? searchTerm, InvoiceStatus? status, ElectronicDocumentType? documentType)
    {
        var query = _dbContext.Invoices
            .AsNoTracking()
            .Where(invoice => invoice.InvoiceType == InvoiceType.Sales && !invoice.IsDeleted);
    ...
    public SalesInvoiceFormViewModel? GetById(Guid id)
    {
        var invoice = _dbContext.Invoices
            .AsNoTracking()
            .Include(invoice => invoice.Items)
            .FirstOrDefault(invoice => invoice.Id == id && invoice.InvoiceType == InvoiceType.Sales && !invoice.IsDeleted);
    ...
    public Guid Add(SalesInvoiceFormViewModel form)
    {
        var invoice = new InvoiceRecord
        {
            ...
            PartyName = Clean(form.CustomerName),
            PartyTaxNumber = Clean(form.CustomerTaxNumber),
    ...
    public bool Update(Guid id, SalesInvoiceFormViewModel form)
    {
        ...
        invoice.PartyName = Clean(form.CustomerName);
        invoice.PartyTaxNumber = Clean(form.CustomerTaxNumber);
    ...
    {
        var invoice = _dbContext.Invoices.FirstOrDefault(invoice => invoice.Id == id && invoice.InvoiceType == InvoiceType.Sales && !invoice.IsDeleted);
    ...
    private string GetNextInvoiceNumber()
    {
        ...
        var count = _dbContext.Invoices.Count(invoice => invoice.InvoiceType == InvoiceType.Sales) + 1;
        string number;

        do
        {
            number = $"SF-{year}-{count:0000}";
            count++;
        }
    ...
    private static SalesInvoiceListItemViewModel ToListItem(InvoiceRecord invoice) =>
        new(
            invoice.Id,
            invoice.IssueDate,
            invoice.PartyName,
    ...
    private static SalesInvoiceFormViewModel ToForm(InvoiceRecord invoice) =>
        new()
        {
            Id = invoice.Id,
            ...
            CustomerTaxNumber = invoice.PartyTaxNumber,
